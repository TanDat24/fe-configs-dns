...
import { wpAddDomainToCurrentUser } from "@/lib/server/wp-domain";
...
export async function POST(request: Request) {
  ...
  const result = await wpAddDomainToCurrentUser(endpoint, token, payload);
  if (!result.ok) {
    logApiError(requestId, "/api/domain/add", result.message, { status: result.status, code: result.code });
