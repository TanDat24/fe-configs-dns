...
import { wpDeleteMyUserContactV2 } from "@/lib/server/wp-customer-data";
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (id <= 0) {
    return errorResponse("id khong hop le.", 400, requestId);
  }

  const result = await wpDeleteMyUserContactV2(endpoint, token, id);
  if (!result.ok) {
