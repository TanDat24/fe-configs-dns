...
import { wpGetOrderItemsV2, wpUpsertOrderItemV2 } from "@/lib/server/wp-customer-data";
...
export async function GET(request: Request, context: { params: Promise<{ orderId: string }> }) {
  ...
  const result = await wpGetOrderItemsV2(endpoint, token, {
    orderId,
...
export async function POST(request: Request, context: { params: Promise<{ orderId: string }> }) {
  ...
  const result = await wpUpsertOrderItemV2(endpoint, token, { ...body, order_id: orderId });
