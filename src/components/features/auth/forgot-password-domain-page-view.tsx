...
import { forgotPasswordByDomain } from "@/lib/api/auth";
...
export function ForgotPasswordDomainPageView() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      const { message } = await forgotPasswordByDomain({ domain, email });
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col bg-gradient-to-b from-zinc-50 to-white">
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-4 py-10 sm:py-14">
        <div className="w-full max-w-[420px] rounded-xl border border-zinc-200/80 bg-white px-7 py-8 shadow-sm sm:px-8 sm:py-9">
          ...
          <h1 className="mb-1 text-center text-lg font-semibold text-zinc-900">
            Khôi phục mật khẩu tên miền
          </h1>
          <p className="mb-5 text-center text-xs text-zinc-500">
            Nhập tên miền và Gmail để nhận hướng dẫn khôi phục mật khẩu
          </p>
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            ...
            <div>
              <input
                ...
                disabled={loading}
                placeholder="Tên miền (ví dụ: example.com)"
